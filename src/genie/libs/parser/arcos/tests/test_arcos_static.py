...
def test_show_static_routing_basic():
    ...
    protocols = result["network_instances"]["default"]["protocols"]
...
def test_show_static_routing_ecmp():
    ...
    routes = result["network_instances"]["default"]["protocols"]["static-routes"]["static_routes"]
    ...
    assert next_hops["nh1"]["next_hop"] == "10.9.201.1"
    assert next_hops["nh1"]["interface"] == "swp1"
    assert next_hops["nh2"]["next_hop"] == "10.9.202.1"
    assert next_hops["nh2"]["interface"] == "swp2"
    assert next_hops["nh3"]["next_hop"] == "10.9.203.1"
...
def test_show_static_routing_drop():
    ...
    assert "1" in next_hops
    nh = next_hops["1"]
    assert nh["next_hop"] == "openconfig-local-routing:DROP"
...
def test_show_static_routing_ipv6():
    ...
    assert "1" in next_hops
    nh = next_hops["1"]
    assert nh["next_hop"] == "2001:db8:1::1"
    assert nh["interface"] == "swp1"
...
def test_show_static_routing_mpls():
    ...
    assert route["local_label_index"] == 100
    
    next_hops = route["next_hops"]
    assert "NH" in next_hops
    nh = next_hops["NH"]
    assert nh["next_hop"] == "10.9.250.1"
    assert nh["interface"] == "swp3"
    assert nh["remote_label_stack"] == [1000, 2000, 3000]
...
def test_show_static_routing_bfd():
    ...
    assert nh["interface"] == "swp3"
    assert "bfd" in nh
    assert nh["bfd"]["destination_address"] == "10.0.0.2"
...
def test_show_static_routing_vrf_leaking():
    ...
    assert "1" in next_hops
    nh = next_hops["1"]
    assert nh["next_network_instance"] == "vrfA"
...
def test_show_static_routing_linklocal_ipv6():
    ...
    assert "next-hop1" in next_hops
    nh = next_hops["next-hop1"]
    assert nh["next_hop"] == "fe80::5054:ff:fef7:8d0e"
    assert nh["interface"] == "swp3"
...
def test_show_static_routing_empty():
    ...
    assert result == {"network_instances": {}}
